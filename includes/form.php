...
/**
 * changes the classes from the div wrapper around each field change the 
 * div class="description" to <small> adds form-required
 */
function occam_form_element($variables) {
  ...
  if(! theme_get_setting('occam_classes_form_wrapper_formitem')){
    $attributes['class'] = array('form-item');
  }*/
  //new:
  // remove .form-item class from field wrapper for label & input
  $attributes['class'] = array();
...
// remove .form-type-[type] class from label & input wrapper
  if (!empty($element['#type'])) {
    //old:
    /*if(!theme_get_setting('occam_classes_form_wrapper_formtype')){
      $attributes['class'][] = 'form-type-' . strtr($element['#type'], '_', '-');
    }*/
    //new:
    $attributes['class'][] = 'form-type-' . strtr($element['#type'], '_', '-');
  }
// remove .form-tye-[name]-x from label & input wrapper  
//old:
/*  if (!empty($element['#name'])) {
    if(!theme_get_setting('occam_classes_form_wrapper_formname')){
  ...
  // Add .form-disabled class to disabled elements 
  if (!empty($element['#attributes']['disabled'])) {
    $attributes['class'][] = 'form-disabled';
  }

  // Add .form-requiered class to requiered elements
  if( $element['#required'] ) {
...
*/
  ...
  if (!empty($element['#description'])) {
     ...
     */
    //old:
    /*if(!theme_get_setting('occam_classes_form_description')){
...
*/
function occam_form_element_label($variables) {
  $element = $variables['element'];
  ...
  // remove hardcoded "*", instead use css to append to label
  //old:
...
//       $attributes['class'] = 'form-field-required';
  ...
  // remove .option from <label> element
  //old:
...
*/
function occam_textfield($variables) {
  $element = $variables['element'];
  $element['#size'] = '30';
  
  // add 'requiered' placeholder text into requiered text fields
  $required = !empty($element['#required']) ? ' required' : '';

  // remove unneccesary type attribute (not needed in HTML5)
  $element['#attributes']['type'] = 'text';

	// add element's title as field placeholder if set true
  if (!empty($element['#title']) AND theme_get_setting('occam_classes_form_placeholder_label') ) {
...
function occam_link_field($vars) {
  ...
  if (empty($element['#field']['multiple'])) {
	...
	//placeholder
	if( theme_get_setting('occam_classes_form_placeholder_link') ){
    $element['#attributes']['placeholder'] = theme_get_setting('occam_classes_form_placeholder_link');
	}
	elseif (!empty($element['#title']) AND theme_get_setting('occam_classes_form_placeholder_label') ) {
   $element['#attributes']['placeholder'] = $element['#title'];
  }
...
*/
function occam_emailfield($variables) {
  $element = $variables['element'];
  ...
  _form_set_class($element, array('form-text', 'form-email'));
	...
	//placeholder
	if( theme_get_setting('occam_classes_form_placeholder_email') ){
    $element['#attributes']['placeholder'] = theme_get_setting('occam_classes_form_placeholder_email');
	}
	elseif (!empty($element['#title']) AND theme_get_setting('occam_classes_form_placeholder_label') ) {
   $element['#attributes']['placeholder'] = $element['#title'];
  }
...
function occam_urlfield($variables) {
  $element = $variables['element'];
...
function occam_textarea($variables) {
  $element = $variables['element'];
...
function occam_checkbox($variables) {
  $element = $variables['element'];
  $t = get_t();
...
function occam_radio($variables) {
  $element = $variables['element'];
...
*/
function occam_password($variables) {
  $element = $variables['element'];
  $element['#size'] = '30';
...
/* removed form-select */
function occam_select($variables) {
  $element = $variables['element'];
  element_set_attributes($element, array('id', 'name', 'size'));
...
remove form-text class
*/
function occam_text_format_wrapper($variables) {
  $element = $variables['element'];
...
function occam_button($variables) {
  $element = $variables['element'];
...
remove form-wrapper
*/
function occam_fieldset($variables) {
  $element = $variables['element'];
...
function occam_container($variables) {
  $element = $variables['element'];
...
function occam_field_multiple_value_form($variables) {
  $element = $variables['element'];
	$output = '';
...
*/
function occam_form_alter(&$form, &$form_state, $form_id) {
/*
	print "<pre>";
	print_r($form_id);
	print_r($form);
